...
public class QuarryT1BlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public QuarryT1BlockEntity(BlockPos position, BlockState state) {
		super(ElecTronicsModBlockEntities.QUARRY_T_1.get(), position, state);
	}
	...
	public Component getDefaultName() {
		return Component.literal("quarry_t_1");
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new QuarryT1GUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
	...
	private final FluidTank fluidTank = new FluidTank(1000, fs -> {
		if (fs.getFluid() == ElecTronicsModFluids.COOLANTT_1.get())
			return true;
		if (fs.getFluid() == ElecTronicsModFluids.REFINED_OIL.get())
			return true;
		return false;
	}) {
		@Override
